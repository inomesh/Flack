document.addEventListener("DOMContentLoaded", () => {
  ...
  socket.on("sending back", (obj) => {
    ...
    // now main
    if (userCheck === true) {
      //this will execute only if user exist in localstorage
      ...
      //this will run if user doesn't exist in localStorage
    ...
    if (obj["channel"] === document.querySelector("#channelName").textContent) {
      ...
      //this will execute if, both user's ain't in the same channel window, but logged in.
      ...
      for (const key in sidebarListItem) {
        if (
          sidebarListItem[key].textContent === obj["channel"] ||
          ...
          if (sidebarListItem[key].childNodes[0].childNodes[1]) {
            count = Number(
              sidebarListItem[key].childNodes[0].childNodes[1].textContent
            );
  ...
  socket.on("newChannel result", (newChannelName) => {
    ...
    if (newChannelName[1] === true) {
      ...
      document.querySelector(".channelInvalidInput").textContent =
        "Channel already exists, please try another name for your channel.";
    }
