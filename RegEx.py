...
def spaces(check):      #function to get gid of spaces and tabs in a sentence
    a = re.compile(r'[\s\t\n]{1,}')
    check = a.sub(' ',str(check))    
    b = check
    return b

# function for email pre-requisite checking
def emailCheck(check):
    ...
    b = re.findall(r'^[\w\W]{5,20}@[\Da-zA-Z]{1,15}\.com$',check)      
...
# function for pasword pre-requisite checking
def pas(check):
    a = re.findall(r'^[\w\W]{8,}$',check)
    if len(a) > 1 or len(a) == 0 or len(a[0]) < 8:
        return False #"try again and make sure your password must have the length of 8 or more."
    else:
        return True
